...
public class Day07_3 {//c s
	...
	public static void main(String[] args) {//m s
		...
		// 3. 프로그램 실행 반복문 
		while(true) { // 무한루프 [ 종료조건 : 3번 입력시 break ] //w s
				...
				for( int i = 0 ; i<memberlist.length ; i++ ) {
					...
					if( memberlist[i][0] !=null && memberlist[i][0].equals(id) && memberlist[i][1].equals(password) ) {
						...
						while(true) { // 로그인시 무한루프 [ 종료조건 : 로그아웃[3] 입력시 ]
							...
							System.out.println("\n[[[ 회원 메뉴 ]]]");
							System.out.print("[[ 1.회원탈퇴 2.비밀번호변경 3.로그아웃 4.게시판 선택 : ");
							...
							if( ch2 == 1 ) {//회원탈퇴
								...
								for( int j = i ; j<memberlist.length; j++) {
									if( memberlist[j+1][0] !=null) {//다음회원 여부 확인
										memberlist[j+1][0] =memberlist[j+1][0];//현재위치에 j에 다음회원 j+1 대입
										memberlist[j+1][1] =memberlist[j+1][1];//현재위치에 j에 다음회원 j+1 대입
									}else {// 다음 회원이 없으면
								...
								System.out.println("[[-----> 새로운 비밀번호 : "); String newpassword = scanner.next();
								...
								System.out.print(" 1.글쓰기  2.글목록 3.이전 페이지  : ");
								int ch3 = scanner.nextInt();
								if( ch3 == 1 ) {// 글쓰기 만들기
									System.out.println("[[글 쓰기]]");
									System.out.print("[[ 제목 : ");		String title = scanner.next();
									System.out.print("[[ 내용 : ");	String contents = scanner.next();
										for( int l = 0 ; l<contentlist.length ;l++) {
											if( contentlist[l][0] != null) {
												// 제목이 null 이라면 
												System.out.println("[[실패]] 제목이 존재하지가 않습니다 ");
												break; // 가장 가까운 반복문 탈출
											}
											else if( contentlist[l][1] != null) {
												// 내용이 null 이라면 
												System.out.println("[[실패]] 내용이 존재하지가 않습니다 ");
												break; // 가장 가까운 반복문 탈출
											}
										...
										for( int l = 0 ; l<contentlist.length; i++ ) {
											if( contentlist[l][0] == null ) {
												...
												System.out.println("[[완료]] : 글 작성이 완료되었습니다");
												break;
											}
										}
								...
								else if( ch3 == 3 ) {System.out.print("[[[이전 페이지로 돌아갑니다]]" );
									break;}
